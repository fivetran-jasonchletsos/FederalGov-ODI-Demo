 ...
 * Ported from Healthcare-EPIC-Snowflake-Demo for Bureau of Federal Outcomes.
 *
 * Components:
 *   LineagePanel         — live-evolving lineage graph for WizardLivePage
 *   BuildCompleteSummary — 4-pane summary for build-complete panel
 *
 * Zero-dep SVG and CSS. Aligned with the BFO navy/red palette.
 */
    ...
    <div className="research-card flex flex-col" style={{ minHeight: 220 }}>
      <header className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--hairline)' }}>
        ...
        <span className="font-mono" style={{ color: 'var(--ink-soft)', fontSize: 12 }}>
          {nodes.filter((n) => n.kind === 'silver').length} gold upstreams — 1 new gold
        </span>
      </header>
      <div className="flex-1 relative" style={{ minHeight: 180, padding: '14px 14px 12px' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" role="img" aria-label="Live model lineage">
          ...
          {nodes.map((n) => {
            const fill = n.kind === 'silver' ? '#cfd6e0' : C.navy;
            const stroke = n.kind === 'silver' ? '#94a3b8' : (currentStep >= 6 || complete) ? C.green : C.navy;
            return (
              <g key={n.id} opacity={nodeOpacity(n)} style={{ transition: 'opacity 700ms ease' }}>
                ...
                <text
                  ...
                  style={{ fontSize: 2.6, fontFamily: '"IBM Plex Mono", monospace', fontWeight: 700 }}
                >
                  {truncate(n.label, 22)}
                </text>
              </g>
          ...
          <text x="2" y="12" fill={C.inkDim} style={{ fontSize: 2.6, letterSpacing: '0.2em', fontFamily: '"IBM Plex Mono", monospace' }}>GOLD UPSTREAM</text>
          <text x="98" y="12" textAnchor="end" fill={C.inkDim} style={{ fontSize: 2.6, letterSpacing: '0.2em', fontFamily: '"IBM Plex Mono", monospace' }}>NEW GOLD</text>
        </svg>
        <div className="absolute left-3 bottom-2 right-3 flex items-center justify-between font-mono" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
          ...
          <span style={{ color: complete ? C.green : C.navy }}>
...
function stepCaption(step: number, complete: boolean): string {
  if (complete) return 'New gold table is live · downstream OIG consumers see it on next read';
  ...
  if (step >= 1) return 'Explorer found candidate gold upstream tables';
          ...
          <div className="mt-3 space-y-2">
            {p.stats.map((s) => (
              <div key={s.label}>
                ...
                <div className="font-semibold" style={{ fontSize: 16, color: 'var(--ink-strong)', lineHeight: 1.2 }}>
                  {s.value}
