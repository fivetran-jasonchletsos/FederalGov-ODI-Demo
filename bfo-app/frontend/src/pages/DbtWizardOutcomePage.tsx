/*
 * DbtWizardOutcomePage — Post-build outcome page for the dbt-wizard demo.
 *
 ...
 *
 * Ported from Healthcare-EPIC-Snowflake-Demo/ClarityOutcomePage.tsx — BFO-flavored.
 */
...
export default function DbtWizardOutcomePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {/* Root-cause panel */}
      <section
        className="rounded-none border border-[var(--hairline)] p-6 mb-10"
        style={{ borderLeft: '5px solid var(--red)', background: 'var(--red-bg)' }}
      >
        <div className="eyebrow mb-2">Root cause identified</div>
        <p className="text-xl sm:text-2xl font-bold leading-tight text-[var(--ink-strong)] mb-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <div className="eyebrow mb-1">Payments affected</div>
          ...
          <div>
            <div className="eyebrow mb-1">Fix required</div>
            <div className="font-mono text-sm font-semibold text-[var(--ink-strong)]">
              Restart the backfill · not a process redesign
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10">
        <LineagePanel
          ...
          subtitle="No gold table tracks recovery by office tier, program, and quarter."
        ...
        <LineagePanel
          ...
          subtitle="Materialized to Iceberg. Downstream OIG consumers attached."
          nodes={o.after.nodes}
          edges={o.after.edges}
          tone="resolved"
        />
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div className="rounded-none border border-[var(--hairline)] bg-white p-5 sm:col-span-2"
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-bold mb-4 pb-2 border-b border-[var(--hairline)] text-[var(--ink-strong)]">
      ...
      {/* Hero */}
      <section
        className="rounded-none border border-[var(--hairline)] p-8 mb-10"
        style={{ borderLeft: '5px solid var(--green)', background: 'var(--green-bg)' }}
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
          <div className="sm:col-span-1">
            <div className="eyebrow mb-2">dbt-wizard result</div>
            <div className="text-6xl sm:text-7xl font-bold tracking-tight text-[var(--green)]">
              {o.hero.value}
            </div>
            <div className="font-mono text-xs mt-2 text-[var(--ink-muted)]">question to materialized</div>
          </div>
          <div className="sm:col-span-2">
            <div className="text-2xl sm:text-3xl font-bold leading-tight text-[var(--ink-strong)]">
              {o.hero.label}
            </div>
      ...
      {/* CTAs */}
      <div className="flex flex-col sm:flex-row gap-3 rounded-none border border-[var(--hairline)] bg-white p-5 items-center justify-between">
        <div>
          <div className="text-2xl font-bold text-[var(--ink-strong)]">Run it again?</div>
        ...
        <div className="flex gap-2 flex-wrap">
          <Link
            to="/"
            className="inline-flex items-center gap-2 rounded-none border border-[var(--hairline)] bg-white text-[var(--ink-strong)] font-bold px-5 py-2.5 hover:bg-[var(--paper-deep)] transition-colors"
          >
            Back to home
          </Link>
          <Link
            to="/dbt-wizard/live"
            className="inline-flex items-center gap-2 rounded-none text-white font-bold px-5 py-2.5 hover:opacity-90 transition-opacity"
...
function WizardColumn({ data, tone }: { data: Column; tone: 'crisis' | 'resolved' }) {
  const toneColor =
    tone === 'crisis' ? 'var(--red)' : 'var(--green)';
  const chipClass =
    tone === 'crisis' ? 'status-pill red' : 'status-pill green';
  return (
    <div
      className="rounded-none border border-[var(--hairline)] bg-white p-6"
      style={{ borderLeft: `5px solid ${toneColor}` }}
    >
      <div className={`${chipClass} mb-3 inline-flex`} style={{ fontSize: 11 }}>
        {data.label}
      </div>
      <h2 className="text-xl font-bold mb-2 text-[var(--ink-strong)]">{data.summary}</h2>

      <dl className="space-y-2 my-5 rounded-none border border-[var(--hairline)] bg-[var(--paper-deep)] p-4">
        {data.metrics.map(m => (
...
function LineagePanel({
  ...
  return (
    <div
      className="rounded-none border border-[var(--hairline)] bg-white p-5"
        ...
        {populated.map(layer => (
          <div key={layer}>
            <div
              ...
              style={{ color: NODE_COLOR[layer] ?? 'var(--navy)' }}
            >
              {layer}
            </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderLeftColor: NODE_COLOR[layer] ?? 'var(--navy)',
                      borderLeftWidth: 3,
                      background: isGap
                        ? 'var(--red-bg)'
                        : isNew
                  ...
                  >
                    <div className="font-mono text-[11px]" style={{ color: NODE_COLOR[layer] ?? 'var(--navy)' }}>
                      {layer}
                    </div>
                    ...
                    {isGap && (
                      <div className="font-mono text-[10px] mt-1 text-[var(--red)]">NOT BUILT</div>
                    )}
                    {isNew && (
                      <div className="font-mono text-[10px] mt-1 text-[var(--green)]">
                        BUILT BY dbt-wizard
                      </div>
                    )}
                  </div>
