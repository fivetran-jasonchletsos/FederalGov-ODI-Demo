/*
 * DbtWizardLivePage — dbt-wizard live-build playback for Bureau of Federal Outcomes.
 *
...
const STEP_DEFS = [
  ...
  { label: 'Data Inspection',      who: 'Worker',       tools: 'warehouse, dbt_show',     insight: 'Q2 tier-3 slice'       },
...
export default function DbtWizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('agents.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch agents.json: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('scenario.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch scenario.json: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('build_script.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch build_script.json: ${r.status}`); return r.json(); }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
      setScenario(s);
      setEvents(b.events);
    ...
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '4px solid var(--t-accent, #4a90d9)',
          borderRadius: '0',
          boxShadow: '0 2px 8px rgba(20,34,63,0.08)',
        }}
      >
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              background: 'rgba(30,110,66,0.1)', color: 'var(--green)', border: '1px solid rgba(30,110,66,0.35)',
            }}
          >
            <span
              style={{
          ...
          <span className="font-mono" style={{ color: 'var(--ink-muted)', fontSize: 13 }}>
            Step{' '}
            <span style={{ color: 'var(--t-accent, #4a90d9)', fontWeight: 700 }}>{currentStep}/{totalSteps}</span>
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                background: complete ? 'var(--green)' : 'var(--t-accent, #4a90d9)',
        ...
        <div className="flex items-center gap-2">
          <button
            className="inline-flex items-center gap-1.5 rounded-none font-bold border transition-colors"
      ...
      <div
        ...
        style={{ borderLeftColor: 'var(--t-accent, #4a90d9)' }}
      >
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="eyebrow" style={{ fontSize: 10, marginBottom: 2 }}>
            Program Integrity · {scenario.timezone_label} · {scenario.requested_by}
          </div>
          <p
            className="font-semibold text-[var(--ink-strong)] leading-snug truncate"
            style={{ fontSize: 16 }}
        ...
        <div className="font-mono text-[var(--ink-muted)] shrink-0" style={{ fontSize: 11 }}>
          Target: <span style={{ color: 'var(--t-accent, #4a90d9)', fontWeight: 700 }}>{scenario.metric_code}</span>
        </div>
      </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  color: active ? 'var(--t-accent, #4a90d9)' : done ? 'var(--green)' : 'var(--ink-soft)',
                }}
              >
        ...
        >
          ...
          >
            {visibleNarr.map((m, idx) => {
              ...
              const color = a?.color ?? AGENT_STEP_COLOR[m.e.from] ?? '#1a365d';
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        ...
                        <span
                          ...
                          style={{
                            ...
                            background: 'rgba(26,54,93,0.10)', color: 'var(--navy)',
        ...
        <section className="flex flex-col gap-3 lg:!h-[calc(100dvh-440px)]" style={{ minHeight: 'max(60vh, 300px)' }}>
          ...
          <div className="research-card flex flex-col" style={{ flex: '1.7 1 0' }}>
            ...
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_recovery_by_office_tier_program_quarter.sql
                </div>
                <span
            ...
            <pre
              ...
              style={{
                ...
                background: '#0e1a2e', color: '#e8edf8',
          ...
          <div className="research-card flex flex-col" style={{ flex: '1 1 0' }}>
            ...
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_recovery_by_office_tier_program_quarter.yml
                </div>
                <span
                  className="layer-chip"
                  style={{
                    color: 'var(--green)', background: 'var(--green-bg)',
                    border: '1px solid rgba(30,110,66,0.3)',
                    fontSize: 10, padding: '3px 8px', fontWeight: 700, whiteSpace: 'nowrap',
                  }}
                >
              ...
              >
                <span
                  style={{
                    ...
                    background: i === 0 ? 'var(--navy)' : 'var(--ink-soft)',
        ...
        >
          <div className="flex items-baseline justify-between flex-wrap gap-3 mb-1">
            <div className="flex items-baseline gap-3 flex-wrap">
              <div
                ...
                style={{
                  ...
                  border: '1px solid rgba(30,110,66,0.35)',
                }}
              >
                Build Complete
              </div>
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-none font-bold transition-colors"
              style={{
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'bfo-odi/wizard-live · dbt-wizard';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        .wizard-terminal [style*="color: var(--navy)"] { color: var(--t-accent) !important; }
        ...
        .wizard-terminal a[class*="rounded-none"] {
          background: var(--t-elev) !important;
          color: var(--t-text) !important;
          border-color: var(--t-line) !important;
        }
        .wizard-terminal button:hover,
        .wizard-terminal a[class*="rounded-none"]:hover {
